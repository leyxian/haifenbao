...
class buyControl extends BaseBuyControl {
    ...
    public function order_totalOp(){
        $res = 'false';
        $address_id = $_POST['address_id'];
        $row = Model()->table('address')->field('id_number')->find($address_id);
        switch ($_POST['type']) {
            case '1':
                $on = 'order.order_id=order_common.order_id';
                $where = 'order.order_flow=10 AND order.order_state>=\'20\' AND order_common.reciver_id=\''.$row['id_number'].'\' AND order.add_time >= '.strtotime('-1 week');
                if($row['id_number']){
                    $row = Model()->table('order,order_common')->join('inner')->on($on)->where($where)->count();
                    if($row < 3) $res = 'true';
                }
                break;
            ...
            default:
                ...
                if($row['id_number']){
                    $on = 'order.order_id=order_common.order_id';
                    $where = 'order.order_flow=10 AND order.order_state >=\'20\' AND order_common.reciver_id = \''.$row['id_number'].'\' AND order.add_time >= '.strtotime(date('Y-m-d'));
                    $row = Model()->table('order,order_common')->join('inner')->on($on)->where($where)->sum('order.goods_amount');
                    $row = $row ? $row : 0;
                    if($row + $num <= 1000) $res = 'true';
                }
                break;
        }
        die($res);
