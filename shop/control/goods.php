...
class goodsControl extends BaseGoodsControl {
	 ...
	 *
	 */
	public function add_shortStockOp() {
		//$state_type	= $_GET['state_type'];
		$goods_id	= intval($_GET['goods_id']);

		$goods_order = Model('goods');
		$condition = array();
		$condition['goods_id'] = $goods_id;
		//$condition['store_id'] = $_SESSION['store_id'];
		$goods_info	= $goods_order->getshortStockGoodsList($condition,'goods_id,goods_name');		
		$goods_info	= $goods_info[0];
		//print_r($goods_info);die;
		Tpl::output('goods_info',$goods_info);
		Tpl::output('goods_id',$goods_id);
		Tpl::showpage('store_add_goods_shortStock','null_layout');			
		
/*		$goods_shortstock = Model('shortstock_goods');
		try {		
		    $goods_shortstock->beginTransaction();

    		    $this->add_goods_shortStock();
    		    //$message = Language::get('store_order_edit_ship_success');
				$message = '添加缺货登记成功!';
				$goods_shortstock->commit();
				showDialog($message,'reload','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');

		} catch (Exception $e) {
		    $goods_shortstock->rollback();
	 ...
	 */
	public function add_goods_shortStockOP() {
	   // $goods_id = $goods_info['goods_id'];		
	    if(isset($_POST) && $_POST['form_submit']=='ok') {
			$goods_order = Model('goods');	
	        $data = array();
			$data['gshort_goodsid'] =intval($_POST['goodsid']);
			$data['gshort_goodsname'] =$_POST['goodsname'];
			$data['gshort_goodquantity'] =intval($_POST['goodquantity']);
			$data['gshort_content'] =$_POST['gcontent'];
			$data['gshort_addtime'] =TIMESTAMP;
			$data['gshort_frommemberid'] =$_SESSION['member_id'];
			$data['gshort_frommembername'] =$_SESSION['member_name'];
			$data['gshort_state'] =1;
			
	        //$data['order_amount'] = array('exp','goods_amount+'.$data['shipping_fee']);
			//print_r($data);	die;
	       $update = $goods_order->addGoods($data,$table = "shortstock_goods");
			
	        if (!$update) {
	            throw new Exception('缺货登记失败.');
	        }
			else
			showDialog('添加缺货登记成功!','reload','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');			
