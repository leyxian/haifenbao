...
<div class="item-publish">
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        <dt><i class="required">*</i>重量<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="g_weight" type="text" class="text w60" value="<?php echo $output['goods']['goods_weight']; ?>" /><em class="add-on"><i class="icon-weight">g</i></em><span></span>
        </dd>
      </dl>
      <dl>
      ...
      <dl>
        ...
        <dd nc_type="no_spec">
          <p>
            <input name="g_serial" type="text"  class="text" value="<?php echo $output['goods']['goods_serial']; ?>" maxlength="50"  />
          </p>
...
$(function(){
    ...
    $('#goods_form').validate({
        ...
        messages : {
            ...
            g_serial : {
                required    : '<i class="icon-exclamation-sign"></i>请选择填写商品唯一编号'
            },			
            image_path : {
