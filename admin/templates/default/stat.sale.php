  ...
  <table class="table tb-type2 nobdb">
    ...
    <?php foreach ($output['order_list'] as $k=>$v){?>
      <tr class="hover">
        ...
        ?></td>
        <td name="mingxi" style="position: relative;">
          <a href="javascript:" >明细</a>
          <table style="position: absolute;background: #fff;border: 1px solid; display:none;right:100px; bottom: 30px; width:500px;">
            <th>产品名称</th>
            <th>数量</th>
            <th>成本价格</th>
            <th>销售价格</th>
            <th>成本合计</th>
            <?php foreach ($v['goods'] as $va) {?>
              <tr>
                <td style="max-width:270px"><?php echo $va['goods_name']; ?></td>
                <td><?php echo $va['goods_num']; ?></td>
                <td><?php echo $va['goods_costprice'];?></td>
                <td><?php echo $va['goods_pay_price']; ?></td>
                <td><?php echo $va['goods_costprice'] * $va['goods_num'];?></td>
              </tr>
            <?php }?>
          </table>
        </td>
...
<script>
$(function () {
  $('td[name="mingxi"]').mousemove(function(){
