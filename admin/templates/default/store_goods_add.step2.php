...
<script type="text/javascript"  src="<?php echo SHOP_SITE_URL;?>/resource/js/store_goods_add.step2.js"></script>
...
<link rel="stylesheet" type="text/css" href="<?php echo ADMIN_TEMPLATES_URL;?>/css/seller_center.css" />
...
<div class="item-publish">
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlAdmin('goods', 'edit_save_goods');} else { echo urlAdmin('goods', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        ...
        <dd id="gcategory"> <?php echo $output['goods_class']['gc_tag_name'];?> <!--<a class="ncsc-btn" href="<?php //if ($output['edit_goods_sign']) { echo urlAdmin('goods', 'edit_class', array('commonid' => $output['goods']['goods_commonid'], 'ref_url' => getReferer())); } else { echo urlAdmin('goods', 'add'); }?>"><?php //echo $lang['nc_edit'];?></a>-->
      ...
      <dl>
        <dt class="required"><i class="required">*</i><?php echo $lang['goods_index_name'].$lang['nc_colon'];?></dt>
        <dd>
...
<dl>
        ...
        <dd>
		  ...
		  <?php if (!empty($output['goods']['goods_state'])){?>
            <li>
              <label>
                <input name="order_state" value="1" type="radio" <?php if ($output['goods']['goods_state']== 1) {?>checked="checked"<?php }?> />
               开启 </label>
            </li> 
            <li>
              <label>
                <input name="order_state" value="0" type="radio" <?php   if ($output['goods']['goods_state'] == 0 || $output['goods']['goods_state'] == 10) {?>checked="checked"<?php }?> />
                下架 </label>
            </li>
			<?php }
			if (empty($output['goods']['goods_state']) ){
			?>
            <li>
              <label>
                <input name="order_state" value="1" type="radio" checked="checked"?> 
               开启 </label>
            </li> 
            <li>
              <label>
                <input name="order_state" value="0" type="radio" />
                下架 </label>
            </li>				
				<?php }	?>		
      ...
      <dl nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        ...
        <dd <?php if($k == '1'){?>nctype="sp_group_val"<?php }?>>
          <ul class="spec">
            ...
            <?php }?>
            <?php }?>
            <li data-param="{gc_id:<?php echo $output['goods_class']['gc_id'];?>,sp_id:<?php echo $k;?>,url:'<?php echo urlAdmin('goods', 'ajax_add_spec');?>'}">
...
<!--                <a class="ncsc-btn" nctype="show_image" href="<?php// echo urlShop('store_album', 'pic_list', array('item'=>'goods'));?>"><i class="icon-picture"></i>从图片空间选择</a> <a href="javascript:void(0);" nctype="del_goods_demo" class="ncsc-btn ml5" style="display: none;"><i class="icon-circle-arrow-up"></i>关闭相册</a>--></div>
      ...
      <dl>
        ...
        <dd>
          <select name="b_id">
            <option value="0"><?php echo $lang['store_goods_step1_please_select'];?></option>
...
var SHOP_RESOURCE_SITE_URL = "<?php echo SHOP_SITE_URL;?>/resource";
...
function recursionSpec($len,$sign) {
    if($len < $sign){
        ...
        echo "str += '<input type=\"hiddean\" name=\"spec['+spec_bunch+'][goods_id]\" nc_type=\"'+spec_bunch+'|id\" value=\"\" />';";
        for($i=0; $i< $len; $i++){
            echo "if (td_".($i+1)."[2] != null) { str += '<input type=\"hidden\" name=\"spec['+spec_bunch+'][color]\" value=\"'+td_".($i+1)."[1]+'\" />';}";
            echo "str +='<td><input type=\"hiddean\" name=\"spec['+spec_bunch+'][sp_value]['+td_".($i+1)."[1]+']\" value=\"'+td_".($i+1)."[0]+'\" />'+td_".($i+1)."[0]+'</td>';\n";
        }
