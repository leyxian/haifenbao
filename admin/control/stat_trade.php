...
class stat_tradeControl extends SystemControl{
    ...
    public function saleOp(){
		...
		//统计数据标题
		$statlist['headertitle'] = array('订单号','买家','下单时间','订单总额','订单状态','查看明细');
		//导出Excel
        if ($_GET['exporttype'] == 'excel'){
			//导出Excel
			import('libraries.excel');
			...
			if($order_list)
				while (list($k, $v) = each($order_list)) {
					$row = Model('order_goods')->where('order_id='.$v['order_id'])->select();
					foreach ($row as $ke => $va) {
						$good = Model()->table('goods,goods_common')->field('goods_common.goods_costprice')->join('inner')->on('goods.goods_commonid=goods_common.goods_commonid')->where('goods.goods_id='.$va['goods_id'])->find();
						$row[$ke]['goods_costprice'] = $good['goods_costprice'];
					}
					$order_list[$k]['goods'] = $row;
				}
			//得到统计图数据
			if(trim($_GET['stat_type']) == 'sale'){
